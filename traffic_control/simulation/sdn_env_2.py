...
class SDN_env(gym.Env):
    def __init__(self, topo_info, env_info, disable_action_reward, seed, result_path, save_results):
        ...
        self.state = self.init_topology_state()
    ...
    def get_link_loads(self):
        ...
        link_loads = []
        for _, link in self.link_loads.items():
            link_loads.append(link['load'] / link['max load'])
        return link_loads
    ...
    def step(self, action: ActType) -> tuple[ObsType, SupportsFloat, bool, bool, dict[str, Any]]:
        ...
        link_loads = self.get_link_loads()

        # calculate rewards
        if self.disable_action_reward is False:
            reward = self.compute_reward(self.state, next_state, link_loads)
        else:
            reward = 0
        ...
        LOGGER.info(f"Step info - link loads: {link_loads}")
    ...
    @staticmethod
    def compute_reward(old_state, new_state, link_loads):
        # switch_penalty = 0
        link_penalty = 0
